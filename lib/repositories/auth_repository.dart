...
class AuthRepository extends ChangeNotifier {
  ...
  AuthRepository(this._apiClient, this._sessionStorage) {
    ...
    // Real conversation calls (list/history/send/toggle/stream) have no
    // auth-recovery path of their own — a dead token there previously just
    // failed silently with no way back to login. Wired to the same guarded
    // handler as above.
    BusinessChatService.onUnauthorized = _handleSessionExpired;
  }
  ...
  Future<void> restoreSession() async {
    ...
    if (session == null) {
      ...
      // wrong for any reason, it would otherwise stay wrong forever — a
      // restart doesn't help, since this exact stale cache is what gets
  ...
  /// SessionStorage) if it's out of date — silent, best-effort, never
  ...
  */
  ...
  Future<void> continueWithEmail(String email) async {
    ...
    _status = AuthStatus.authenticated;
    AppLogger.i('AuthRepository',
        'continuing with email $trimmed (no backend account)');
    notifyListeners();
  }

  Future<void> continueAsGuest() async {
    const guest =
        User(id: 'guest', name: 'Guest', username: '', roleName: 'guest');
  ...
  /// recovery: the stale token and the businessId cached alongside it
  /// (session_storage.dart) just stayed in place and 401'd again on the next
  /// call, indefinitely. Clearing both and returning to login — the same way
  /// a fresh install starts — is the real fix; a bare retry would 401 again
  /// with the same token, and a fresh login re-resolves businessId from
  /// scratch rather than trusting the stale cached one.
